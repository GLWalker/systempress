...
if (!function_exists('sp_enqueue_block_parts_dark_mode')) {
    ...
    function sp_enqueue_block_parts_dark_mode()
    {
        ...
        $css->add_property('z-index', '9999999');

        $css->set_selector('.dark-mode p:not(#nm-theme)');
        $css->add_property('display', 'none !important');

        $css->set_selector('.dark-mode p');
        ...
        $css->set_selector('svg.bi');
        $css->add_property('margin-right', 'var(--wp--preset--spacing--20)');

        $css->set_selector('.theme-icon-active');
        ...
        $css->add_property('padding', 'var(--wp--preset--spacing--10) !important');
        ...
        <script>
            (() => {
                ...
                const showActiveTheme = (theme, focus = false) => {
                    const themeSwitcher = document.querySelector('#nm-theme')

                    if (!themeSwitcher) {
                        return
                    }

                    const themeSwitcherText = document.querySelector('#nm-theme-text')
...
if (!function_exists('sp_editor_styles_dark_mode')) {
    ...
    function sp_editor_styles_dark_mode($css)
    {
        ...
        $css->set_selector('#nm-theme');
        $css->add_property('margin', '0');
        $css->add_property('vertical-align', 'middle');

        $css->set_selector('#nm-theme:before');
        $css->add_property('content', '"\263C"');
        $css->add_property('padding-inline-start', '0.125rem');
        $css->add_property('vertical-align', 'middle');
        $css->add_property('display', 'inline-block');
        $css->add_property('font-size', '1rem');
