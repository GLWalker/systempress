 ...
 *
 * @since 0.0.1
 * @source GeneratePress forked from GeneratePress theme by Tom Usborne.
 */
...
if ( !function_exists( 'sp_get_media_query' ) ) {
	 ...
	 */
	function sp_get_media_query( $name )
	{


		$xxl = apply_filters( 'sp_xxl_media_query', '( max-width:1320px )' );

		$xl = apply_filters( 'sp_xl_media_query', '( max-width:1140px )' );

		$lg = apply_filters( 'sp_lg_media_query', '( max-width:960px )' );

		$md = apply_filters( 'sp_md_media_query', '( max-width:720px )' );
		$sm = apply_filters( 'sp_sm_media_query', '( max-width:540px )' );
	...
	class SystemPress_CSS
	{
		...
		private function add_selector_rules_to_output()
		{
			if ( !empty( $this->_css ) ) {
				...
				if ( 'rawout' !== $this->_selector_output ) {
					$selector_output = sprintf( '%1$s{%2$s}', $this->_selector_output, $this->_css );
				} else {
					$selector_output = $this->_css;
				}
